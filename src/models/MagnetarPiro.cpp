...
MagnetarPiro::MagnetarPiro(shared_ptr<Cosmology> cosmology, shared_ptr<Filters> filters, shared_ptr<Absorption> absorption) : SNModel(cosmology, filters, absorption) {
    ...
    uParams_ = {5e14, 10.0, 100, 20, 20, 50, 1e52};
...
void MagnetarPiro::calcSEDParams(double t) {
    ...
    SEDParams_[1] = pow(lumMag(t-modelParams_[5]) / (CGS_SIGMA * 4 * M_PI * pow(radius(t), 2)), 0.25);
    SEDParams_[2] = pow(lumPiro(t) / (CGS_SIGMA * 4 * M_PI * pow(radius(t), 2)), 0.25);
}
...
double MagnetarPiro::calcSED(double wav) {
    ...
    res *= (1.0/(exp(CGS_H * CGS_C / (wav * 1e-8 * CGS_K * SEDParams_[1])) - 1.0)) + (1.0/(exp(CGS_H * CGS_C / (wav * 1e-8 * CGS_K * SEDParams_[2])) - 1.0));
